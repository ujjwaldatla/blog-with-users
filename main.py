...
from wtforms.validators import DataRequired, Email as email, Length
...
# Defining the decorator for admin-only routes
def admin_only(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # If the user is not authenticated or not an admin (assuming admin has id=1)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        email_duplication_check = db.session.execute(db.select(User).filter_by(email=email))
        if email_duplication_check.scalar_one_or_none():
